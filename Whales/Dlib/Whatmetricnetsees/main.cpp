...
int main(int argc, char ** argv) try
{
    ...
    cv::Mat _cvmat = loadIFgraymatWsize(_cmd.get<string>("image"),_imgsize.width,_imgsize.height,false,true,true,&_loaded);
    if(_loaded == false) {
    ...
    if(_cmd.has("reference")) {
        _cvrefmat = loadIFgraymatWsize(_cmd.get<string>("reference"),_imgsize.width,_imgsize.height,false,true,true,&_loaded);
        if(_loaded == false) {
    ...
    _dlibmatrices.push_back(cvmat2dlibmatrix<float>(_cvrefmat)); // reference image
    for(unsigned int i = 0; i < _cmd.get<unsigned int>("rows"); ++i) {
        for(unsigned int j = 0; j < _cmd.get<unsigned int>("cols"); ++j) {
            _dlibmatrices.push_back(cvmat2dlibmatrix<float>(cutoutRect(_cvmat,j*_xstep,i*_ystep,0.1f,0.1f)));
        }
    }
    cout << "Inference..." << endl;
