...
cv::Point mkpoint(const dlib::point &_p)
{
    return cv::Point(_p.x(),_p.y());
}
...
int main(int argc, char** argv)
{
    try
    {
        ...
        load_image_dataset(images_train, faces_train, "/home/alex/Testdata/Flukes/Train/Flukes.xml");
        load_image_dataset(images_test, faces_test,   "/home/alex/Testdata/Flukes/Test/Flukes.xml");
        ...
        directory testdir(faces_directory);
        auto file = testdir.get_files();
        for(size_t i = 0; i < file.size(); ++i) {

            std::cout << i << ") " << file[i].name() << std::endl;

            cv::Mat _mat = cv::imread(file[i].full_name(),CV_LOAD_IMAGE_GRAYSCALE);
            if(_mat.empty() == false) {
                dlib::matrix<uchar> _img = cvmat2dlibmatrix<uchar>(_mat);

                dlib::rectangle _bbox(_mat.cols,_mat.rows);
                dlib::full_object_detection _shape = sp(_img,_bbox);
                std::cout << sp.num_parts() << std::endl;

                cv::cvtColor(_mat,_mat,CV_GRAY2BGR);
                for(int j = 0; j < _shape.num_parts(); ++j) {
                    cv::circle(_mat,mkpoint(_shape.part(j)),3,cv::Scalar(0,255,0),1,CV_AA);
                }
