...
int main(int argc, char *argv[])
{
    ...
    for(const auto &_filename: files) {
        ...
        _headpose.angles = std::vector<float>(2,0);
        QStringList _sections = _filename.section('.',0,0).split(QRegularExpression("\\b"));
        _headpose.angles[0] = QString("%1%2").arg(_sections.at(2),_sections.at(3)).toFloat() / 90.0f;
        _headpose.angles[1] = QString("%1%2").arg(_sections.at(4),_sections.at(5)).toFloat() / 90.0f;
        //qDebug() << _sections << _headpose.angles;
    ...
    qInfo("Pan: %.2f plus/minus %.2f deg", mean_err_pan, 2.0f*stdev_err_pan);
    const float mean_err_tilt = 90.0f*mean(dtilt), stdev_err_tilt = 90.0f*stdev(dtilt);
    qInfo("Tilt: %.2f plus/minus %.2f deg", mean_err_tilt, 2.0f*stdev_err_tilt);
