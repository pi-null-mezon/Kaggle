...
int main(int argc, char** argv) try
{
    ...
    dlib::array<dlib::matrix<dlib::rgb_pixel>> vcrops;
    size_t _nc = 5;


    size_t _cannotload = 0;
    for(size_t j = 1; j <= 12800; ++j) { // cause test data contains 12800 entities
        _filename = std::to_string(j);
        cout << j << ") filename " << _filename;
        img = std::move(load_rgb_image_with_fixed_size(std::string(argv[3]) + std::string("/") + _filename + std::string(".jpg"),IMGSIZE,IMGSIZE,false,&_isloaded));
        if(_isloaded) {
            //load_image(img,std::string(argv[3]) + std::string("/") + _filename + std::string(".jpg"));
            //randomly_crop_image(img,vcrops,_rnd,_nc,0.80,0.99,IMGSIZE,IMGSIZE);
            randomly_jitter_image(img,vcrops,0,_nc,IMGSIZE,IMGSIZE,1.2,0.4,10.0);
            matrix<float,1,CLASSES> p = sum_rows(mat(snet(vcrops.begin(), vcrops.end())))/vcrops.size();
            _label = lblname[index_of_max(p)]; // shift is caused by different indexing
    ...
    cout << "There is " << _cannotload << " images have not been loaded" << endl;
}
