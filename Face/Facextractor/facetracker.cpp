...
cv::RotatedRect FaceTracker::searchFace(const cv::Mat &img)
{
    ...
    switch(m_primaryfacedetectortype) {
        case FaceTracker::ViolaJones:
            ...
            pt_faceClassifier->detectMultiScale(image.getUMat(cv::ACCESS_FAST), rects, 1.21, m_minNeighbours, cv::CASCADE_FIND_BIGGEST_OBJECT, m_minFaceSize, m_maxFaceSize );
            break;
    ...
    if(faceRect.area() > 0) {
        ...
        switch(m_method) {
            ...
            case FaceShape: {
                ...
                m_angle += 180.0 * std::atan(peyes.y / peyes.x) / PI_VALUE; // 180.0 produces angle jitter, and 90.0 looks more stable
            }
            break;
        }
    }
    ...
    if(faceImg.cols > m_minFaceSize.width && faceImg.rows > m_minFaceSize.height)  {
        ...
        if(roiRect.area() > 0)  {
            ...
            if(size.area() > roiRect.area())
                interpolationMethod = cv::INTER_LANCZOS4;
            else
...
void FaceTracker::setPrimaryFaceDetectorType(FaceTracker::PrimaryFaceDetector _type)
{
    m_primaryfacedetectortype = _type;
}
...
{
    m_minFaceSize = size;
    m_maxFaceSize = cv::Size(size.width*10, size.height*10);
