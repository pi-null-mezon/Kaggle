#ifndef METRIC_COS_LOSS_H
#define METRIC_COS_LOSS_H
...
template <
    typename EXP
    >
typename disable_if_c<std::numeric_limits<typename EXP::type>::is_integer, const typename EXP::type>::type cos_distance (
...
public:
        ...
        ) const
    {
        ...
        for (long r = 0; r < temp.num_samples(); ++r)
        {
            ...
            for (long c = r+1; c < temp.num_samples(); ++c)
            {
                ...
                else
                {
                    ...
                    auto dcos = 1.0 - xy / std::sqrt(xx * yy);
                    if (dcos <= 0)
                        dcos = 0;
                    temp_threshs.push_back(dcos);
                }
            }
        }
        ...
        for (long r = 0; r < temp.num_samples(); ++r)
        {
            ...
            for (long c = 0; c < temp.num_samples(); ++c)
            {
                ...
                // compute the distance between x and y samples.
                auto dcos = 1.0f - xy / std::sqrt(xx * yy);
                if(dcos <= 0)
                    dcos = 0;

                // It should be noted that the derivative of dcos(x,y) = 1 - xy / (|x||y|) with respect
                ...
                if (x_label == y_label)
                {
                    ...
                    if (dcos < dist_thresh-margin)
                    {
                        gm[r*temp.num_samples() + c] = 0;
                    }
                    else
                    {
                        loss += scale*(dcos - (dist_thresh-margin));
                ...
                else
                {
                    ...
                    // them.  If not then we experience non-zero loss.
                    if (dcos > dist_thresh+margin || dcos > neg_thresh)
                    {
                        gm[r*temp.num_samples() + c] = 0;
                    }
                    else
                    {
                        loss += scale*((dist_thresh+margin) - dcos);
			// don't divide by zero (or a really small number)
                        auto _tmp = std::max(xx * std::sqrt(xx * yy), 0.001f);
                        gm[r*temp.num_samples() + r] -= scale * xy / _tmp;
                        gm[r*temp.num_samples() + c] = scale * xx / _tmp;
    ...
    friend void serialize(const loss_metric_cos_& item, std::ostream& out)
    {
        serialize("loss_metric_cos_", out);
        serialize(item.margin, out);
        serialize(item.dist_thresh, out);
    }

    friend void deserialize(loss_metric_cos_& item, std::istream& in)
    {
        std::string version;
        deserialize(version, in);
        if (version == "loss_metric_cos_" || version == "loss_metric_cossim_")
        {
        ...
        {
            throw serialization_error("Unexpected version found while deserializing dlib::loss_metric_cos_.  Instead found " + version);
        }
    }

    friend std::ostream& operator<<(std::ostream& out, const loss_metric_cos_& item )
    {
        out << "loss_metric_cossim (margin="<<item.margin<<", distance_threshold="<<item.dist_thresh<<")";
        return out;
    }

    friend void to_xml(const loss_metric_cos_& item, std::ostream& out)
    {
        out << "<loss_metric_cossim margin='"<<item.margin<<"' distance_threshold='"<<item.dist_thresh<<"'/>";
    }
...
using loss_metric_cos = add_loss_layer<loss_metric_cos_, SUBNET>;

}

#endif // METRIC_COS_LOSS_H
