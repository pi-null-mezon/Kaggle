    ...
    for(size_t i = 0; i < num_classes; ++i) {
        ...
        for(size_t j = 0; j < samples_per_class; ++j) {
            ...
            if(_doaugmentation) {
                ...
                bool resdown = false;
                if(rnd.get_random_float() > 0.9f && (id != 0)) {
                    int side = rnd.get_integer_in_range(40,100);
                    cv::resize(_tmpmat,_tmpmat,cv::Size(side,side));
                    resdown = true;
                }
                ...
                if(rnd.get_random_float() > 0.5f) { // only for attack labels
...
const cv::String options = "{traindir  t  |       | path to directory with training data}"
                           "{cvfolds      |   4   | folds to use for cross validation training}"
