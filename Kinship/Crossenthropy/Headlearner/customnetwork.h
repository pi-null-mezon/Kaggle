...
using net_type =  loss_multiclass_log< fc<2, relu<dropout<  fc<512,relu<dropout  <fc<1024, bn_fc<input<matrix<float>>>>> >>> >>>;

// testing network type (replaced batch normalization with fixed affine transforms)
using anet_type = loss_multiclass_log< fc<2, relu<multiply< fc<512,relu<multiply <fc<1024, affine<input<matrix<float>>>>> >>> >>>;


// testing network type (replaced batch normalization with fixed affine transforms)
using dlib_face_dscr_type = loss_metric<fc_no_bias<128,avg_pool_everything<
                            alevel0<
                            alevel1<
                            alevel2<
                            alevel3<
