...
int main(int argc, char** argv) try
{
    ...
    for(int n = 0; n < cmdparser.get<int>("number"); ++n) {
        ...
        dlib::pipe<std::pair<image_info,matrix<rgb_pixel>>> data(512);
            ...
            while(data.is_enabled())
            {
                ...
                while(_classes < number_of_classes) {
                    ...
                    if(_vsoc_selected[temp.first.numeric_label] == false) {
                        ...
                        size_t num_crops = 1;
                        float _rndval = rnd.get_random_float();
                        if(_rndval < 0.5f) {
                            randomly_crop_image(img,crops,rnd,num_crops,IMGSIZE,IMGSIZE);
                            if(_rndval < 0.0f)
                                randomly_cutout_rect(crops[0],crops,rnd,num_crops);
                        }
                        else if(_rndval < 1.0f) {
                            randomly_jitter_image(img,crops,seed,num_crops,IMGSIZE,IMGSIZE);
                            if(_rndval < 0.0f)
                                randomly_cutout_rect(crops[0],crops,rnd,num_crops);
                        }
        ...
        dlib::pipe<std::pair<image_info,matrix<rgb_pixel>>> validationdata(128);
