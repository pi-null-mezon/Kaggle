...
void load_image_with_normalization(matrix<float>& image, const std::string& file_name);

#define FILTERS 16

using net_type = loss_multiclass_log<softmax<fc<2,avg_pool_everything<dropout<
...
void load_image_with_normalization(matrix<float>& image, const std::string& file_name) {
    ...
    cv::Mat _mat = cv::imread(file_name, CV_LOAD_IMAGE_UNCHANGED);
    //std::cout << "channels: " << _mat.channels() << "\tCV_DEPTH: " << _mat.depth() << std::endl;

    _mat.convertTo(_mat,CV_32F);
    //double _min,_max;
    //cv::minMaxIdx(_mat,&_min,&_max);
    //std::cout << "min: " <<  _min << "\tmax: " << _max << std::endl;
    cv::Mat _vchannelmean, _vchannelstdev;
    cv::meanStdDev(_mat,_vchannelmean,_vchannelstdev);
    //std::cout << "mean:" << _vchannelmean.at<const double>(0) << "\tstdev:" << _vchannelstdev.at<const double>(0) << std::endl;;
    cv::Mat _nmat = (_mat - _vchannelmean.at<const double>(0)) / _vchannelstdev.at<const double>(0);

    image = cvmat2dlibmatrix<float>(_nmat);
/*
    cv::Mat _vismat(num_rows(image), num_columns(image), CV_32FC1, image_data(image));
    cv::namedWindow("Probe",CV_WINDOW_NORMAL);
    cv::imshow("Probe", _vismat);
    cv::waitKey(0);
*/
}
...
const cv::String keys =
       ...
       "{swptrain         | 6000   | determines after how many steps without progress (training loss) decay should be applied to learning rate  }"
...
int main(int argc, char** argv) try
{
    ...
    for(int n = 0; n < cmdparser.get<int>("number"); ++n) {
        ...
        cout << "Data split (train / test): " << trainingset.size() << " / " << validationtset.size() << endl;
        ...
        std::vector<matrix<float>> samples, validationsamples;
        ...
        dlib::pipe<std::pair<image_info,matrix<float>>> data(512);
            ...
            while(data.is_enabled())
            {
                ...
                while(_classes < number_of_classes) {
                    ...
                    if(_vsoc_selected[temp.first.numeric_label] == false) {
                        ...
                        _classes++;
                        load_image_with_normalization(img,temp.first.filename);
                        dlib::array<matrix<float>> crops;
                        size_t num_crops = 1;
                        if(rnd.get_random_float() < 0.5f)
                            randomly_crop_image(img,crops,rnd,num_crops);
                        else
        ...
        dlib::pipe<std::pair<image_info,matrix<float>>> validationdata(128);
            ...
            while(validationdata.is_enabled())
            {
                ...
                while(_classes < number_of_classes) {
                    ...
                    if(_vsoc_selected[temp.first.numeric_label] == false) {
                        ...
                        dlib::array<matrix<float>> crops;
                        size_t num_crops = 1;
                        if(rnd.get_random_float() < 0.5f) // take in mind that this is validation images preprocessing
                            randomly_crop_image(img,crops,rnd,num_crops);
                        else
                            randomly_jitter_image(img,crops,seed,num_crops);
                        temp.second = std::move(crops[0]);
        ...
        static const double _min_learning_rate_thresh = cmdparser.get<double>("minlrthresh");
        while(trainer.get_learning_rate() > _min_learning_rate_thresh)
        {
            ...
            if(trainer.get_train_one_step_calls() % 10 == 0) { // Now we can perform validation test
                ...
                std::pair<image_info, matrix<float>> validationimg;
                while(validationsamples.size() < 32) {
        ...
        if(trainer.get_average_test_loss() < cmdparser.get<double>("lossthresh")) {
            ...
            {
                dlib::array<matrix<float>> images;
                matrix<float> img;
                load_image_with_normalization(img, l.filename);
                ...
                randomly_jitter_image(img,images,0,num_crops);
                ...
                _logloss += -std::log(p(l.numeric_label));
            ...
            serialize(cmdparser.get<std::string>("outputdirpath") + "/net_" + std::to_string(n) + "_(" + std::to_string(trainer.get_average_loss())  + " - " + std::to_string(trainer.get_average_test_loss()) + " - " + std::to_string(_logloss/validationtset.size()) + ").dat") << net;
            cout << "Average logloss: " << _logloss/validationtset.size() << endl;
