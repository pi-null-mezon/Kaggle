    ...
    for(size_t i = 0; i < num_classes; ++i) {
        ...
        for(size_t j = 0; j < samples_per_class; ++j) {
            ...
            if(_doaugmentation) {
                ...
                if(rnd.get_random_float() > 0.5f)
                    _tmpmat = jitterimage(_tmpmat,cvrng,cv::Size(0,0),0.075,0.075,7.5,cv::BORDER_CONSTANT,cv::Scalar(0),false);
                if(rnd.get_random_float() > 0.5f)
                ...
                if(rnd.get_random_float() > 0.5f)
                    _tmpmat *= static_cast<double>(0.6f + 0.8f*rnd.get_random_float());
...
const cv::String options = "{traindir  t  |       | path to directory with training data}"
                           ...
                           "{classes c    | 9     | classes per minibatch}"
    ...
    for(size_t _fold = 0; _fold < allobjsfolds.size(); ++_fold) {
        ...
        while(trainer.get_learning_rate() >= cmdparser.get<double>("minlrthresh"))  {
            ...
            if((trainer.get_train_one_step_calls() % 1000) == 0) {
                ...
                net.clean();
                if(testobjs.size() > 0) {
                    /*cout << "Evaluation on VLIDATION set:" << endl;
                    test_accuracy_on_set(validobjs,net,128,true);*/
