    ...
    for(size_t i = 0; i < num_classes; ++i) {
        ...
        for(size_t j = 0; j < samples_per_class; ++j) {
            ...
            if(_doaugmentation) {
                ...
                if(rnd.get_random_float() > 0.5f)
                    _tmpmat = jitterimage(_tmpmat,cvrng,cv::Size(0,0),0.025,0.025,2.5,cv::BORDER_CONSTANT,cv::Scalar(0),false);
...
int main(int argc, char** argv)
{
    ...
    for(size_t _fold = 0; _fold < allobjsfolds.size(); ++_fold) {
        ...
        if(testobjs.size() > 0) {
            ...
            acc = test_accuracy_on_random_subset(testobjs,net,true,classes_per_minibatch,4);
            cout << " avg random subset accuracy: " << acc << endl;
