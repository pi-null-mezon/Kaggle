    ...
    for(size_t i = 0; i < num_classes; ++i) {
        ...
        for(size_t j = 0; j < samples_per_class; ++j) {
            ...
            if(_doaugmentation) {
                ...
                if(id == 1) // Posterize
                    _tmpmat = posterize(_tmpmat,static_cast<uint8_t>(rnd.get_integer_in_range(2,16)));

                if(rnd.get_random_float() > 0.5f)
                    cv::flip(_tmpmat,_tmpmat,1);

                if(rnd.get_random_float() > 0.5f)
                    _tmpmat = jitterimage(_tmpmat,cvrng,cv::Size(0,0),0.05,0.05,10,cv::BORDER_REPLICATE,cv::Scalar(0),false);
                if(rnd.get_random_float() > 0.5f)
                ...
                if(rnd.get_random_float() > 0.5f) {
                    float scale = 0.5f + 2.5f*rnd.get_random_float();
        ...
        while(trainer.get_learning_rate() >= cmdparser.get<double>("minlrthresh"))  {
            ...
            if((trainer.get_train_one_step_calls() % 1000) == 0) {
                ...
                if(testobjs.size() > 0) {
                    ...
                    float acc = test_accuracy_on_set(testobjs,net);
                    if(acc > 0.98) {
